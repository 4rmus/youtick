...
export default function Error({ error }: ErrorProps) {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-black px-4">
        <div className="max-w-md w-full text-center">
          <div className="w-12 h-12 mx-auto border-4 border-near-green border-t-transparent rounded-full animate-spin mb-6" />
          <p className="text-zinc-400">{copy.loading_retrying}</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-black px-4">
      <div className="max-w-md w-full text-center">
        <div className="mb-8">
          <div className="w-16 h-16 mx-auto bg-near-red/10 rounded-full flex items-center justify-center">
            <svg className="w-8 h-8 text-near-red" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        {error.digest && (
          <p className="text-xs text-zinc-500 mb-6">{copy.error_id}: {error.digest}</p>
        )}
