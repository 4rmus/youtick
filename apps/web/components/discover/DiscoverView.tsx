...
export function DiscoverView() {
    ...
    return (
        <div className="container mx-auto min-h-[calc(100vh-4rem)] px-4 py-10">
            <div className="mb-8">
                <p className="text-sm uppercase tracking-[0.24em] text-emerald-300">Livepeer publications</p>
                <h1 className="mt-3 text-3xl font-bold">Discover</h1>
                <p className="mt-3 text-sm text-zinc-400">Paid video releases finalized on NEAR.</p>
            </div>

            {query.loading ? (
                <div role="status" className="flex items-center gap-3 text-zinc-300"><Loader2 className="h-5 w-5 animate-spin" /> Loading publications…</div>
            ) : query.error ? (
                ...
                <p className="rounded-lg border border-zinc-800 p-10 text-center text-zinc-400">No active publications yet.</p>
            ) : (
                <>
