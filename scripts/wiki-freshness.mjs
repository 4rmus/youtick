...
const lines = [];
lines.push(`# LLM Wiki tazelik raporu (${today})`);
lines.push('');
lines.push(`- Stale aday: ${stale.length} | Yaslanmis: ${aged.length} | Taze: ${fresh.length}`);
lines.push('');

lines.push('## Stale aday (kaynak last_checked sonrasi degisti)');
lines.push('');
if (stale.length === 0) {
  lines.push('Yok.');
} else {
  lines.push('| Sayfa | last_checked | En yeni kaynak |');
  lines.push('|---|---|---|');
  for (const e of stale) lines.push(`| [[${e.wikiName}]] | ${e.lastChecked} | ${e.newestSource} |`);
}
lines.push('');

lines.push('## Yaslanmis (esik asildi)');
lines.push('');
if (aged.length === 0) {
  lines.push('Yok.');
} else {
  lines.push('| Sayfa | last_checked | Yas (gun) | Esik |');
  lines.push('|---|---|---|---|');
...
if (write) {
  const page = [
    ...
    '---',
    '',
    '> Bu sayfa `node scripts/wiki-freshness.mjs --write` ile uretilir; elle duzenleme.',
    '',
    report,
    '## Ilgili sayfalar',
    '',
    ...
    '## Sonraki check',
    '',
    '- Stale aday satirlarini ingest workflow ile yeniden dogrula ve `last_checked` guncelle.',
    '',
  ].join('\n');
