...
| Y6 | near-sdk upgrade denendi (5.5.0→5.26.1) → **BLOKE**. access-control + operator-registry 5.26.1'de temiz derledi + test 4+4 gecti. nft-ticket'in sandbox (`near-workspaces`) test agi kirildi: 5.26 WASM yeni host fn (`promise_batch_action_deploy_global_contract`) uretiyor → workspaces 0.14 sandbox VM'de yok. Workspaces 0.22 sandbox'u desteklior ama transitive deps (`time@0.3.47`, `home@0.5.12`, vb.) rust 1.88+ istiyor. rust 1.87+ ile derlenen WASM'i ise `cargo-near-build 0.10` reddediyor ("nearcore VM ile uyumsuz"). Kilidini acabilmek icin workspaces ekibinin hem eski rust uyumu hem yeni sandbox kombini saglamasi gerekiyor. **Geri alindi** — tum kontratlar 5.5.0 (effektif 5.24.1 cargo-lock) baseline'ina restore edildi. Deploy engeli degil; 15/15 unit + 33 sandbox yeniden gecti. | 🚫 bloke |
